...
def add_reminder(uid, t, dt, rt='none'):
    with sqlite3.connect(DB_PATH) as c:
        cur = c.cursor()
        cur.execute("INSERT INTO reminders (user_id, title, reminder_datetime, repeat_type) VALUES (?,?,?,?)", (uid, t, dt, rt))
        c.commit()
        return cur.lastrowid

def get_reminders(uid, act=True):
    with sqlite3.connect(DB_PATH) as c:
        q = "SELECT id, title, reminder_datetime, repeat_type FROM reminders WHERE user_id=? AND is_active=1 ORDER BY reminder_datetime" if act else "SELECT id, title, reminder_datetime, repeat_type FROM reminders WHERE user_id=? ORDER BY reminder_datetime"
        return c.execute(q, (uid,)).fetchall()

def get_upcoming(uid, lim=10):
    now = datetime.now(pytz.UTC).strftime("%Y-%m-%d %H:%M:%S")
    with sqlite3.connect(DB_PATH) as c:
        return c.execute("SELECT id, title, reminder_datetime, repeat_type FROM reminders WHERE user_id=? AND is_active=1 AND reminder_datetime > ? ORDER BY reminder_datetime LIMIT ?", (uid, now, lim)).fetchall()
...
def parse_dt(txt):
    ...
    for n, w in wd.items():
        if txt.startswith(n):
            tm = re.search(r'([0-1]?[0-9]|2[0-3]):([0-5][0-9])', txt)
            if tm:
...
# ========== ХЕНДЛЕРЫ ==========
async def start_cmd(update, ctx): register_user(update.effective_user.id, update.effective_user.username, update.effective_user.first_name); await update.message.reply_text(f"✨ *Привет!* Я бот для напоминаний.\n🌍 Ваш пояс: *{get_user_tz(update.effective_user.id).zone}*\n\n➕ `/add` | 📋 `/myevents` | ⌚ `/settz`", parse_mode="Markdown")
async def settz_cmd(update, ctx): await update.message.reply_text("🌍 Выберите часовой пояс:", parse_mode="Markdown", reply_markup=tz_kb())
async def create_start(update, ctx): q = update.callback_query; await q.answer(); await q.edit_message_text("📝 *Новое напоминание*\n\nВведите название:", parse_mode="Markdown", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Отмена", callback_data="cancel")]])) if q else await update.message.reply_text("📝 *Новое напоминание*\n\nВведите название:", parse_mode="Markdown"); return WAITING_TITLE
async def handle_title(update, ctx): t = update.message.text.strip(); 
    if not t: await update.message.reply_text("❌ Название не может быть пустым."); return WAITING_TITLE
    ctx.user_data['rem_title'] = t; await update.message.reply_text(f"✅ *{t}*\n\n📅 Введите дату и время:\n• `15:30` — сегодня\n• `завтра 18:00`\n• `31.12 20:00`\n• `пн 10:00`", parse_mode="Markdown"); return WAITING_DATETIME
async def handle_dt(update, ctx): dt = parse_dt(update.message.text.strip()); 
...
async def handle_repeat(update, ctx): q = update.callback_query; await q.answer(); rt = q.data.replace("repeat_",""); rtx={"none":"один раз","daily":"ежедневно","weekly":"еженедельно","monthly":"ежемесячно"}.get(rt,"один раз"); uid=update.effective_user.id; t=ctx.user_data.get('rem_title'); ldt=ctx.user_data.get('rem_dt'); tz=get_user_tz(uid); utc=tz.localize(ldt, is_dst=True).astimezone(pytz.UTC); add_reminder(uid, t, utc.strftime("%Y-%m-%d %H:%M:%S"), rt); await schedule_rem(uid, t, utc.strftime("%Y-%m-%d %H:%M:%S"), utc, rt, ctx); ctx.user_data.clear(); await q.edit_message_text(f"✅ Готово!\n\n📝 {t}\n📅 {ldt.strftime('%d.%m в %H:%M')} ({tz.zone})\n🔄 {rtx}", parse_mode="Markdown", reply_markup=main_kb()); return ConversationHandler.END
async def list_rem(update, ctx): q = update.callback_query; mf = q.edit_message_text if q else update.message.reply_text; await q.answer() if q else None; uid=update.effective_user.id; tz=get_user_tz(uid); rems=get_reminders(uid); 
...
async def upcoming_rem(update, ctx): q = update.callback_query; mf = q.edit_message_text if q else update.message.reply_text; await q.answer() if q else None; uid=update.effective_user.id; tz=get_user_tz(uid); rems=get_upcoming(uid); 
...
async def my_events(update, ctx): uid=update.effective_user.id; tz=get_user_tz(uid); rems=get_reminders(uid); 
    if not rems: return await update.message.reply_text("📭 Пока пусто. Создайте: ➕ или `/add`", parse_mode="Markdown")
    ...
    lines.append("\n💡 Удалить: откройте «📋 Мои» и нажмите на напоминание"); await update.message.reply_text("\n".join(lines), parse_mode="Markdown")
async def view_rem(update, ctx): q = update.callback_query; await q.answer(); rid=int(q.data.split("_")[1]); uid=update.effective_user.id; tz=get_user_tz(uid); rem=get_reminder(rid, uid); 
...
async def del_rem(update, ctx): q = update.callback_query; await q.answer(); rid=int(q.data.split("_")[1]); uid=update.effective_user.id; del_reminder(rid, uid); s=ctx.bot_data.get('scheduler'); 
    if s: 
...
async def help_cmd(update, ctx): q = update.callback_query; mf = q.edit_message_text if q else update.message.reply_text; await q.answer() if q else None; await mf("*❓ Помощь*\n\n📅 `15:30` / `завтра 18:00` / `31.12 20:00`\n🌍 Пояс: кнопка ⌚ или `/settz`\n📍 GPS: нажмите «Авто (GPS)» в меню поясов\n🔄 Повтор: один раз / ежедневно / еженедельно / ежемесячно\n📋 Команды: `/start` • `/add` • `/myevents` • `/settz` • `/help`", parse_mode="Markdown", reply_markup=main_kb() if q else None)
async def back_main(update, ctx): q = update.callback_query; await q.answer(); await q.edit_message_text("🏠 Главное меню", parse_mode="Markdown", reply_markup=main_kb())
async def cancel_cmd(update, ctx): q = update.callback_query; 
    if q: await q.answer(); await q.edit_message_text("❌ Отменено", parse_mode="Markdown", reply_markup=main_kb())
    else: await update.message.reply_text("❌ Отменено", parse_mode="Markdown", reply_markup=main_kb())
    ctx.user_data.clear(); return ConversationHandler.END
async def request_gps(update, ctx): q = update.callback_query; await q.answer(); kb=[[KeyboardButton("📍 Отправить геолокацию", request_location=True)]]; await q.message.reply_text("📍 Нажмите кнопку ниже, чтобы бот определил пояс автоматически:", reply_markup=ReplyKeyboardMarkup(kb, one_time_keyboard=True, resize_keyboard=True)); ctx.user_data['waiting_for_gps_tz']=True
async def handle_gps(update, ctx): 
    if not ctx.user_data.get('waiting_for_gps_tz') or not update.message.location: return
    lat=update.message.location.latitude; lon=update.message.location.longitude; tz_name=tf.timezone_at(lat=lat, lng=lon)
    await update.message.reply_text("⌨️ Клавиатура скрыта", reply_markup=ReplyKeyboardRemove()); ctx.user_data.pop('waiting_for_gps_tz', None)
    ...
    else: await update.message.reply_text("❌ Не удалось определить пояс. Выберите вручную:", reply_markup=tz_kb())

async def button_dispatch(update, ctx):
    q = update.callback_query; d = q.data; await q.answer()
    if d=="list_reminders": await list_rem(update,ctx)
    elif d=="upcoming_reminders": await upcoming_rem(update,ctx)
    elif d=="set_tz_btn": await q.edit_message_text("🌍 *Выберите часовой пояс:*", parse_mode="Markdown", reply_markup=tz_kb())
    elif d=="gps_request": await request_gps(update,ctx)
    ...
    elif d=="back_to_main": await back_main(update,ctx)
    elif d=="cancel": await cancel_cmd(update,ctx)
    elif d.startswith("view_"): await view_rem(update,ctx)
    elif d.startswith("delete_"): await del_rem(update,ctx)
    elif d.startswith("repeat_"): await handle_repeat(update,ctx)
...
# ========== ASGI ENTRY POINT (ИМЕННО ЭТО ИЩЕТ ПЛАТФОРМА) ==========
@asynccontextmanager
...
# 🔥 ЯВНОЕ ОПРЕДЕЛЕНИЕ ТОЧЕК ВХОДА
app = Starlette(lifespan=lifespan, routes=[Route("/api/webhook", webhook_handler)])
application = app  # Для совместимости с Render/Heroku
handler = app      # Для совместимости с Vercel/Serverless
