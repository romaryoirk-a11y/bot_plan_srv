...
if not BOT_TOKEN:
    raise ValueError("❌ Переменная BOT_TOKEN не задана в Vercel Environment Variables")
...
def init_db():
    with sqlite3.connect(DB_PATH) as conn:
        conn.execute('''CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, first_name TEXT, timezone TEXT DEFAULT 'Europe/Moscow', created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP)''')
        try: conn.execute("ALTER TABLE users ADD COLUMN timezone TEXT DEFAULT 'Europe/Moscow'")
        except: pass
        conn.execute('''CREATE TABLE IF NOT EXISTS reminders (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, title TEXT, reminder_datetime TEXT, repeat_type TEXT DEFAULT 'none', is_active INTEGER DEFAULT 1, created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP, FOREIGN KEY (user_id) REFERENCES users (user_id))''')
        conn.commit()
...
def get_reminders(uid, act=True):
    with sqlite3.connect(DB_PATH) as c: q="SELECT id, title, reminder_datetime, repeat_type FROM reminders WHERE user_id=? AND is_active=1 ORDER BY reminder_datetime" if act else "SELECT id, title, reminder_datetime, repeat_type FROM reminders WHERE user_id=? ORDER BY reminder_datetime"
...
sched.start()
app_ptb.bot_data['scheduler'] = sched

# Регистрация хендлеров (копируем логику из оригинала)
...
async def handle_title(u, c): t=u.message.text.strip(); return (await u.message.reply_text("❌ Название не может быть пустым.") or WAITING_TITLE) if not t else (c.user_data.__setitem__('rem_title', t) or await u.message.reply_text(f"✅ *{t}*\n\n📅 Введите дату и время:\n• `15:30` — сегодня\n• `завтра 18:00`\n• `31.12 20:00`\n• `пн 10:00`", parse_mode="Markdown") or WAITING_DATETIME)
async def handle_dt(u, c): dt=parse_dt(u.message.text.strip()); return (await u.message.reply_text("❌ Неверный формат. Примеры: `15:30`, `завтра 18:00`", parse_mode="Markdown") or WAITING_DATETIME) if not dt else (c.user_data.__setitem__('rem_dt', dt) or await u.message.reply_text(f"✅ {dt.strftime('%d.%m в %H:%M')}\n\n🔄 Выберите повтор:", parse_mode="Markdown", reply_markup=repeat_kb()) or WAITING_REPEAT)
...
async def handle_gps(u, c): 
    ...
    lat=u.message.location.latitude; lon=u.message.location.longitude; tz_name=tf.timezone_at(lat=lat, lng=lon)
...
app_ptb.add_handler(CommandHandler("settz", settz_cmd))
app_ptb.add_handler(CommandHandler("help", help_cmd))
app_ptb.add_handler(CommandHandler("myevents", my_events))
app_ptb.add_handler(conv)
app_ptb.add_handler(CallbackQueryHandler(lambda u,c: button_dispatch(u,c)))
app_ptb.add_handler(MessageHandler(filters.LOCATION, handle_gps))
...
# ========== ASGI ENTRY POINT ДЛЯ VERCEL ==========
@asynccontextmanager
async def lifespan(app):
    await app_ptb.initialize()
    await app_ptb.start()
    await app_ptb.bot.set_my_commands([BotCommand("start","🚀 Старт"), BotCommand("add","➕ Создать"), BotCommand("myevents","📋 Мои"), BotCommand("settz","⌚ Часовой пояс"), BotCommand("help","❓ Помощь")])
...
async def webhook_handler(request):
    if request.method == "POST":
        ...
        if update: await app_ptb.process_update(update)
